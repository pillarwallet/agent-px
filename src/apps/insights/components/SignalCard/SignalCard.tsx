...
export const SignalCard = forwardRef<HTMLDivElement, SignalCardProps>(
  (
    { signal, leverage, sparklineData, logoMap, animateOnMount = true },
    ref
  ) => {
      ...
      >
          ...
          <div className="flex items-center gap-2 md:gap-3">
                ...
                return logoUrl ? (
                  <>
                    <img
                      ...
                      onError={(e) => {
                        console.error(
                          `❌ Failed to load logo for ${signal.ticker} (${symbol}) from URL: ${logoUrl}, showing fallback`
                        );
                    ...
                    />
                    <span className="text-lg md:text-xl font-bold text-primary hidden items-center justify-center">
                      {symbol.substring(0, 1)}
                    </span>
                  </>
                ) : (
                  <span className="text-lg md:text-xl font-bold text-primary flex items-center justify-center">
                    {symbol.substring(0, 1)}
                  </span>
                );
              })()}
            ...
            <div>
              ...
              <Badge
                className={
                  isOpen
                    ? 'bg-violet-500/20 text-violet-400 border-violet-500/30 ml-2'
                    : 'bg-slate-500/20 text-slate-400 border-slate-500/30 ml-2'
                }
              >
        ...
        <TooltipProvider>
          <div className="grid grid-cols-3 gap-2 md:gap-4 mb-3 md:mb-4">
              ...
              <TooltipTrigger asChild>
                <div className="cursor-help">
                  ...
                  <p className="text-sm md:text-lg font-semibold text-white">
                    $
                    {formatPrice(signal.entry_price, signal.ticker).replace(
                      '.',
                      ','
                    )}
                  </p>
            ...
            <div>
              ...
              <p className="text-sm md:text-lg font-semibold text-white">
                ...
                {signal.current_price
                  ? formatPrice(signal.current_price, signal.ticker).replace(
                      '.',
                      ','
                    )
                  : 'N/A'}
            ...
            <Tooltip>
                ...
                <div className="cursor-help">
                  ...
                  {(() => {
                    ...
                    return isTrailing ? (
                      ...
                      <p className="text-sm md:text-lg font-semibold text-rose-400">
                        $
                        {formatPrice(signal.stop_loss, signal.ticker).replace(
                          '.',
                          ','
                        )}
                      </p>
              ...
              <TooltipContent>
                <p>
                  If price drops to this level, the remaining position closes to
                  limit losses
                </p>
              </TooltipContent>
            </Tooltip>
        ...
        <TooltipProvider>
          <div className="mb-3 md:mb-4">
            <Tooltip>
              <TooltipTrigger asChild>
                <p className="text-[10px] md:text-xs text-muted-foreground mb-2 cursor-help inline-block">
                  Take Profit Targets (33.33% of position each)
                </p>
              </TooltipTrigger>
              <TooltipContent>
                <p>
                  Each target automatically sells 33.33% of the position when
                  reached, locking in profits
                </p>
              </TooltipContent>
            </Tooltip>
            <div className="grid grid-cols-3 gap-1.5 md:gap-2">
              {[
                ...
                .map(({ key, price, hit }) => {
                  const isClosed = ['completed', 'stopped', 'closed'].includes(
                  ...
                  const tpPercent =
                    signal.order_side?.toLowerCase() === 'sell'
                      ? ((signal.entry_price - price) / signal.entry_price) *
                        100
                      : ((price - signal.entry_price) / signal.entry_price) *
                        100;
                  const lockedInPercent = isHit ? tpPercent * 0.3333 : null;
                  const displayedLockedIn =
                    lockedInPercent !== null
                      ? applyLeverage(lockedInPercent).toFixed(2)
                      : null;
                  const displayedPotential = applyLeverage(
                    tpPercent * 0.3333
                  ).toFixed(2);
                  ...
                  return (
                    <Tooltip key={key}>
                      <TooltipTrigger asChild>
                        <div
                          className={`rounded-2xl p-2 md:p-2.5 border cursor-help transition-all duration-300 ${isHit ? 'bg-emerald-500/20 border-emerald-500/40 glow-violet' : 'bg-card/30 border-primary/10 hover:border-primary/30'}`}
                        >
                          ...
                          <p className="text-xs md:text-sm font-semibold text-white">
                            $
                            {formatPrice(price, signal.ticker).replace(
                              '.',
                              ','
                            )}
                          </p>
                          {isHit ? (
                            <p className="text-[10px] md:text-xs text-emerald-400 mt-0.5 font-bold">
                              +{displayedLockedIn}% ✓
                            </p>
                          ) : (
                      ...
                      <TooltipContent>
                        <p>
                          {isHit
                            ? `Locked in +${displayedLockedIn}% profit`
                            : `Target: +${(tpPercent * leverage).toFixed(4)}% gain (33.33% of position = +${displayedPotential}%)`}
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  );
              ...
              {timeline.map((event, idx) => {
                ...
                return (
                  ...
                  >
                    {IconComponent && (
                      <IconComponent
                        className={`w-5 h-5 ${event.iconColor} mt-0.5`}
                      />
                    )}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="text-right">
                          ...
                          {event.pnl !== null && event.pnl !== undefined && (
                            <div
                              className={`text-xs ${event.pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                            >
                              {applyLeverage(event.pnl) > 0 ? '+' : ''}
                              {applyLeverage(event.pnl).toFixed(2)}%
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {isOpen ? (
            <>
              <div>
                ...
                >
                  {applyLeverage(signal.profit_loss_percent || 0) >= 0
                    ? '+'
                    : ''}
                  {applyLeverage(signal.profit_loss_percent || 0)
                    .toFixed(2)
                    .replace('.', ',')}
                  %
                  {(signal.profit_loss_percent || 0) >= 0 && (
            ...
            <>
              <div>
                ...
                >
                  {applyLeverage(signal.realized_pnl_percent || 0) >= 0
                    ? '+'
                    : ''}
                  {applyLeverage(signal.realized_pnl_percent || 0)
                    .toFixed(2)
                    .replace('.', ',')}
                  %
                  {(signal.realized_pnl_percent || 0) >= 0 && (
