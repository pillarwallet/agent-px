...
export const PnLDetailModal = ({
  ...
  const chartData = useMemo(() => {
    ...
    if (view === 'open') {
      ...
      return closedSignals.map((s) => {
        ...
        return {
          ts: new Date(
            s.closed_at || s.last_price_update || s.created_at
          ).getTime(),
          value: cumulative,
          label: s.ticker,
        };
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    const dateText = p.isNow
      ? 'Now'
      : format(new Date(p.ts), 'MMM dd, yyyy HH:mm');
    ...
    return (
      <div className="glass-card p-4 rounded-2xl border border-primary/20 backdrop-blur-2xl">
        ...
        <p
          className={`text-sm font-bold ${val >= 0 ? 'text-[hsl(142,76%,58%)]' : 'text-[hsl(348,83%,58%)]'}`}
        >
          {val >= 0 ? '+' : ''}
          {val.toFixed(2)}%
        </p>
                ...
                <defs>
                  <linearGradient id="colorPnL" x1="0" y1="0" x2="0" y2="1">
                    <stop
                      offset="5%"
                      stopColor={
                        isPositive ? 'hsl(142,76%,58%)' : 'hsl(348,83%,58%)'
                      }
                      stopOpacity={0.3}
                    />
                    <stop
          ...
          <div className="mb-6 glass-card rounded-3xl p-4 border border-amber-500/20">
            ...
            <div className="space-y-6">
              {filteredSignals
                ...
                .map((signal) => {
                  const { events, latestStop } =
                    normalizeTrailingHistory(signal);

                  return (
                    <div
                      key={signal.id}
                      className="border-l-2 border-amber-500/30 pl-3"
                    >
                      ...
                      <div className="space-y-2">
                              ...
                              <div key={idx} className="space-y-1">
                                <div className="flex items-center gap-2 text-xs">
                                  <span className="text-amber-400">
                                    🎯 {event.tp_level?.toUpperCase()} hit
                                  </span>
                                  {event.tp_price && (
                        ...
                        <div className="flex items-center gap-2 text-xs pt-2 border-t border-amber-500/20">
                          ...
                          <span className="font-mono text-white font-semibold">
                            ${formatPrice(latestStop, signal.ticker)}
                          </span>
                        </div>
                      </div>
                    </div>
                                  ...
                                  <details className="text-[10px] text-muted-foreground cursor-pointer">
                                    ...
                                    <div className="mt-2 space-y-1 text-[9px] bg-background/50 rounded p-2">
                                      ...
                                      {(() => {
                                        ...
                                        if (remaining > 0) {
                                          const exitPrice = signal.stop_loss_hit
                                            ? getEffectiveStopLoss(signal)
            ...
            <DrawerDescription className="text-xs flex flex-col gap-1">
              {(() => {
                ...
                return (
                  <>
                    <span>
                      {count} total {count === 1 ? 'trade' : 'trades'}
                    </span>
