    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            ws.onopen = () => {
                ...
                console.log('[Ticker] Sending allMids subscription:', JSON.stringify(midsMsg));
        ...
        const fetchMarketData = async () => {
            try {
                ...
                if (data && data.length >= 2) {
                    const assetCtx = data[1]?.find((ctx: any) => ctx.coin === selectedAsset.symbol);
                    if (assetCtx) {
                        const prevDayPx = parseFloat(assetCtx.prevDayPx || '0');
                        const markPx = parseFloat(assetCtx.markPx || '0');
                        const change24h = markPx - prevDayPx;
                        const changePercent24h = prevDayPx > 0 ? ((change24h / prevDayPx) * 100).toFixed(2) : '0.00';
    ...
    return (
        <div className="flex items-center gap-6 px-4 py-2 bg-card/50 border-b border-border text-sm">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-base">{selectedAsset.symbol}-USDC</span>
                <span className="text-xs text-muted-foreground">{selectedAsset.maxLeverage}x</span>
            </div>

            {/* Mark Price */}
            <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Mark</span>
                <span className="font-semibold">{parseFloat(tickerData.markPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>

            {/* Oracle Price */}
            <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Oracle</span>
                <span className="font-semibold">{parseFloat(tickerData.oraclePrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
            ...
            <div className="flex flex-col">
                ...
                <span className={`font-semibold ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
                    {tickerData.change24h} / {isPositive ? '+' : ''}{tickerData.changePercent24h}%
                </span>
            </div>
            ...
            <div className="flex flex-col">
                ...
                <span className="font-semibold">${parseFloat(tickerData.volume24h).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
            </div>

            {/* Open Interest */}
            <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Open Interest</span>
                <span className="font-semibold">${parseFloat(tickerData.openInterest).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>

            {/* Funding Rate */}
            <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Funding / Countdown</span>
                <span className="font-semibold">{(parseFloat(tickerData.fundingRate) * 100).toFixed(4)}% {tickerData.nextFundingTime}</span>
            </div>
        </div>
    );
}
